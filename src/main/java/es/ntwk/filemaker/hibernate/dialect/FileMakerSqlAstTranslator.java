...
/**
 * A SQL AST translator for FileMaker.
 *
 * @author Francesc Sans
 */
public class FileMakerSqlAstTranslator<T extends JdbcOperation> extends AbstractSqlAstTranslator<T> {

	private static final Logger logger = LoggerFactory.getLogger(FileMakerSqlAstTranslator.class);
	...
	// FileMaker does not allow CASE expressions where all result arms contain plain parameters.
	...
	public void visitOffsetFetchClause(QueryPart queryPart) {
		logger.debug("visitOffsetFetchClause");
		logger.debug("visitOffsetFetchClause OffsetClauseExpression: {}", queryPart.getOffsetClauseExpression());
		logger.debug("visitOffsetFetchClause FetchClauseExpression: {}", queryPart.getFetchClauseExpression());
		logger.debug("visitOffsetFetchClause FetchClauseType: {}", queryPart.getFetchClauseType());


		
		// FileMaker only supports the OFFSET and FETCH clause with ROWS
		assertRowsOnlyFetchClauseType(queryPart);
		logger.debug("visitOffsetFetchClause -> Rows-only fetch clause type verified");
		
		if (supportsOffsetFetchClause()) {
			logger.debug("visitOffsetFetchClause -> Offset/Fetch clause supported, rendering clause");
			renderOffsetFetchClause(queryPart, true);
			logger.debug("visitOffsetFetchClause -> OFFSET/FETCH clause rendered for: {}", queryPart.getOffsetClauseExpression());
		}
		else if (!getClauseStack().isEmpty()) {
			logger.debug("visitOffsetFetchClause -> Subquery detected, OFFSET/FETCH not supported in subqueries");
	...
	protected void renderFetchExpression(Expression fetchExpression) {
		logger.debug("renderFetchExpression  -> " + fetchExpression.getColumnReference() );
		if ( supportsParameterOffsetFetchExpression() ) {
			super.renderFetchExpression( fetchExpression );
			logger.debug("renderFetchExpression -> supportsParameterOffsetFetchExpression" );
		}
		else {
			logger.debug("renderFetchExpression -> not supportsParameterOffsetFetchExpression" );
	...
	protected void renderOffsetExpression(Expression offsetExpression) {
		logger.debug("renderOffsetExpression " + offsetExpression.toString() );
		if ( supportsParameterOffsetFetchExpression() ) {
			super.renderOffsetExpression( offsetExpression );
			logger.debug("renderOffsetExpression -> supportsParameterOffsetFetchExpression"  );
		}
		else {
			logger.debug("renderOffsetExpression -> literal value: " + offsetExpression.toString());
			renderExpressionAsLiteral( offsetExpression, getJdbcParameterBindings() );
			logger.debug("renderOffsetExpression -> not supportsParameterOffsetFetchExpression" );
		}
	}
	...
	private boolean supportsOffsetFetchClause() {
		// FileMaker did support OFFSET and FETCH
		return true;
	}
